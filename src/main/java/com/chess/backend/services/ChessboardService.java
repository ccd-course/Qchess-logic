...
//                [position.getX()][position.getY()];
    ...
    public static ArrayList<Integer> getAllPossibleActions(Chessboard chessboard, Player player){
    ...
    public static int moveToAction(Chessboard chessboard, Square from_square, Square destination_square){
        ...
        // Loop through all action codes for the from_position
        Position test_position = from_pos;
        for(int i = 0; i < max_action_codes + 1; i++){
            ...
            // Some actions taken from the from-position are not possible
            if(test_position == null){
                test_position = from_pos;
                continue;
            }
            // Check, if the position which is generated by applying actions to the from-position would end up
            // in the destination position
    ...
    public static ExecutedMoveObject actionToMove(Chessboard chessboard, int action, int gameID){
        int max_action_codes = 10 * chessboard.getNumberOfPlayers() + 31;
        int action_code = action % max_action_codes;
        int from_pos_id = action / max_action_codes;

        Position from_position = Position.from_id(from_pos_id, 5);
        ...
        int base = 32;
        int[] bases = {0, 4, 8, 12, 16, 17, 18, 19, 20, 21, 22, 23, 24, 28, 32};
        for (int i:
             bases) {
            if (action_code <= i){
                base = i;
        ...
        Position to_position = new Position(from_position.getX(), from_position.getY());

        for (; action_code >= base; action_code--){

            to_position = (action_code < 4) ? to_position.diagFR(chessboard) :
                    (action_code < 8) ? to_position.diagBR(chessboard) :
                    (action_code < 12) ? to_position.diagBL(chessboard) :
                    (action_code < 16) ? to_position.diagFL(chessboard) :
                    (action_code < 17) ? to_position.knightFFR(chessboard) :
                    (action_code < 18) ? to_position.knightFRR(chessboard) :
                    (action_code < 19) ? to_position.knightBRR(chessboard) :
                    (action_code < 20) ? to_position.knightBBR(chessboard) :
                    (action_code < 21) ? to_position.knightBBL(chessboard) :
                    (action_code < 22) ? to_position.knightBLL(chessboard) :
                    (action_code < 23) ? to_position.knightFLL(chessboard) :
                    (action_code < 24) ? to_position.knightFFL(chessboard) :
                    (action_code < 28) ? to_position.right(chessboard) :
                    (action_code < 32) ? to_position.left(chessboard) :
                    (action_code >= 32) ? to_position.forward(chessboard) : to_position;

        }
        int[] previousPiecePosition = {from_position.getX(), from_position.getY()};
        int[] newPiecePosition = {to_position.getX(), to_position.getY()};
        return new ExecutedMoveObject(gameID, previousPiecePosition, newPiecePosition);
    }
